...
def _validate_current_file():
    ...
    if Check.part():
        ...
        try:
            Check.equal("prestejStevila([1, 2, 3])", 3)
            Check.equal("prestejStevila([])", 0)
            Check.equal("prestejStevila([1, [2], 3, [2, 3, 4]])", 6)
            Check.equal("prestejStevila([[[[2]], 1], [2, [[3], [4]]]])", 5)
            Check.equal("prestejStevila([[[1]]])", 1)
            Check.equal("prestejStevila([1, [[[2]], [3]]])", 3)
        except:
    ...
    if Check.part():
        ...
        try:
            Check.equal("prestejNegativnaStevila([-11, -2, -3])", 3)
            Check.equal("prestejNegativnaStevila([])", 0)
            Check.equal("prestejNegativnaStevila([1, [-2], 3, [-2, 3, 4]])", 2)
            Check.equal("prestejNegativnaStevila([[[[2]], 1], [2, [[3], [4]]]])", 0)
            Check.equal("prestejNegativnaStevila([[[-1]]])", 1)
            Check.equal("prestejNegativnaStevila([-1, [[[2]], [-3]]])", 2)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
    ...
    if Check.part():
        ...
        try:
            Check.equal("sestejStevila([1, 2, 3])", 6)
            Check.equal("sestejStevila([])", 0)
            Check.equal("sestejStevila([1, [2], 3, [2, 3, 4]])", 15)
            Check.equal("sestejStevila([[[[2]], 1], [2, [[3], [4]]]])", 12)
            Check.equal("sestejStevila([[[1]]])", 1)
            Check.equal("sestejStevila([-1, [[[2]], [3]]])", 4)
            Check.equal("sestejStevila([1, [-2], -3, [2, 3, 4]])", 5)
            Check.equal("sestejStevila([[[[-2]], -1], [-2, [[-3], [-4]]]])", -12)
            Check.equal("sestejStevila([[[-1]]])", -1)
            Check.equal("sestejStevila([1, [[[2]], [-3]]])", 0)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
    ...
    if Check.part():
        ...
        try:
            Check.equal("jeSSCS([1, 2, 3])", True)
            Check.equal("jeSSCS([])", True)
            Check.equal("jeSSCS(True)", False)
            Check.equal("jeSSCS([1, [2], 3, [2, 3, 4]])", True)
            Check.equal("jeSSCS([1, [2.0], 3, [2, 3, 4]])", False)
            Check.equal("jeSSCS([[[[2]], 1], [2, [[3.6], [4]]]])", False)
            Check.equal("jeSSCS([[[1]]])", True)
            Check.equal("jeSSCS([1])", True)
            Check.equal("jeSSCS([True])", False)
            Check.equal("jeSSCS([-1, 'bla', [[[2]], [3]]])", False)
            Check.equal("jeSSCS(1)", False)
            Check.equal("jeSSCS([1.8])", False)
            Check.equal("jeSSCS([[[[-2, 9, 'x']], -1], [-2, [[-3], [-4]]]])", False)
            Check.equal("jeSSCS([[[-1, 9, 9, 8]]])", True)
            Check.equal("jeSSCS([1, [[[2]], [-3]]])", True)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
