...
def _validate_current_file():
    ...
    if Check.part():
        ...
        try:
            ...
            Check.generator('ogrlice(0, 4)', ["RRRR"], should_stop=True)
            Check.generator('ogrlice(5, 2)', ['BBBBBRR', 'BBBBRBR', 'BBBBRRB', 'BBBRBBR', 'BBBRBRB', 'BBBRRBB', 'BBRBBBR', 'BBRBBRB', 'BBRBRBB', 'BBRRBBB'], further_iter=11, should_stop=True)
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
